...
{
    internal class DataKioskRepository<T> : IDataKioskRepository<T> where T : class
    {

        private readonly IDataKioskUoWork _context;
        private readonly DbSet<T> _dbSet;
        private readonly DbContext _dbContext;

        public DataKioskRepository(DataKioskUoWork uoWork)
        {
            if (uoWork == null) throw new ArgumentNullException("uoWork");
            _context = uoWork;
            _dbSet = uoWork.Context.DataKioskDbContext.Set<T>();
            _dbContext = uoWork.Context.DataKioskDbContext;
        }
        ...
        public T Update(T entity)
        {
            ...
            _context.Context.DataKioskDbContext.Entry(entity).State = EntityState.Modified;
            return updated;
        }
        ...
        public IEnumerable<T> UpdateRange(List<T> entities)
        {
            ...
            foreach (var item in entities)
            {
                ...
                _context.Context.DataKioskDbContext.Entry(item).State = EntityState.Modified;
                retVals.Add(updated);
            }
            return retVals;
        }
